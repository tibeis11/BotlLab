...
export default function Footer({
  ...
  if (variant === 'minimal') {
    return (
      <footer
        className={`pt-12 pb-6 text-center opacity-40 hover:opacity-100 transition-opacity duration-500 flex flex-col items-center border-t border-zinc-900 ${className}`}
      >
        <div className="mb-2">
          <Logo className="w-5 h-5" textSize="text-xs" />
        </div>
        <p className="text-[9px] text-zinc-600 font-medium mt-1">Digital Brew Lab</p>
        <nav className="mt-4 flex gap-4 flex-wrap justify-center" aria-label="Footer Navigation">
          <Link
            href="/impressum"
            className="text-[10px] text-zinc-600 hover:text-zinc-400 hover:underline transition"
          >
            Impressum
          </Link>
          <Link
        ...
        <p className="text-[8px] text-zinc-800 mt-3 font-mono">
          © {year} BotlLab
        </p>
      </footer>
    );
  ...
  return (
    <footer
      className={`border-t border-zinc-900 bg-black py-12 px-4 ${className}`}
    >
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
        ...
        {/* Right: Links */}
        <nav
          className="flex flex-wrap justify-center gap-6 text-sm font-bold text-zinc-400"
        ...
        >
          ...
          <span className="text-zinc-800 select-none hidden md:inline">|</span>

    ...
    return (
        <>
            ...
            <div className="relative overflow-hidden bg-surface border-b border-border">
                <div className="absolute inset-0 bg-gradient-to-r from-warning-bg/20 via-background/80 to-transparent" />
                <div className="relative px-6 md:px-12 lg:px-16 py-5 md:py-8 max-w-screen-2xl mx-auto">
                    <div className="flex items-center gap-2 text-sm text-text-muted mb-1">
                        ...
                        <span>/</span>
                        <span className="text-text-secondary font-medium">Gespeichert</span>
                    </div>
                ...
                <main className="flex-1 min-w-0 px-6 pt-6 space-y-5">
                                ...
                                >
                                    ...
                                    {count > 0 && (
                                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-black
                                            ${activeTab === key ? 'bg-surface-hover text-text-secondary' : 'bg-surface-hover/50 text-text-disabled'}`}>
                                            {count}
                                        </span>
                                    )}
                                    {activeTab === key && (
                                        <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-warning rounded-full" />
                                    )}
                                </Link>
                            );
                        })}
                    ...
                    {filtered.length === 0 && (
                        <div className="text-center py-20">
                            <Bookmark size={36} className="mx-auto text-text-disabled mb-4" />
                            <h3 className="font-bold text-text-secondary mb-1">Nichts gespeichert</h3>
                            ...
                            <Link
                                href="/forum"
                                className="inline-flex items-center gap-2 bg-success hover:bg-success/90 text-text-primary px-4 py-2 rounded-xl text-sm font-bold transition"
                            >
                                Forum entdecken
                            </Link>
                        </div>
                                ...
                                if (item.targetType === 'thread' && item.thread) {
                                    return (
                                        <div key={item.bookmarkId} className="relative">
                                            ...
                                            <div className="absolute top-3 right-3 z-10 flex items-center gap-1 text-[10px] text-warning/70 font-medium">
                                                <Bookmark size={9} className="fill-warning/50" />
                                ...
                                if (item.targetType === 'post' && item.post) {
                                    ...
                                    return (
                                        <Link
                                            key={item.bookmarkId}
                                            href={threadLink}
                                            className="block group border border-transparent hover:border-border hover:bg-surface/50 rounded-xl px-4 py-4 transition-all"
                                        >
                                            <div className="flex items-start justify-between gap-3 mb-2">
                                                <div className="flex items-center gap-2 min-w-0">
                                                    <MessageSquare size={13} className="text-text-disabled shrink-0" />
                                                ...
                                                <div className="flex items-center gap-1 text-[10px] text-warning/70 font-medium shrink-0">
                                                    <Bookmark size={9} className="fill-warning/50" />
                                                    {formatRelativeTime(item.bookmarkedAt)}
                                                </div>
                                            </div>

                                            <p className={`text-sm leading-relaxed line-clamp-3 ${isDeleted ? 'italic text-text-disabled' : 'text-text-secondary group-hover:text-text-primary'}`}>
                                            ...
                                            <div className="flex items-center gap-2 mt-2.5">
                                                {post.author?.logo_url && (
                                                    <img
                                                        ...
                                                        className="w-4 h-4 rounded-full object-cover border border-border-hover"
                                                        alt=""
                                                    />
                                                )}

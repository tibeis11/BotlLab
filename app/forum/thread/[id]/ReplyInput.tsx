        ...
        <div className="sticky bottom-0 z-20 bg-background/95 backdrop-blur-sm border-t border-border/50
            ...
            {replyTarget && (
                <div className="md:hidden flex items-center gap-2 mb-2 px-1">
                    <Reply size={11} className="text-success shrink-0" />
                    <span className="text-xs text-text-secondary truncate">
                        Antwort auf <span className="text-text-secondary font-medium">{replyTarget.authorName}</span>
                    </span>
                    <button
                        onClick={() => setReplyTarget(null)}
                        className="ml-auto text-text-disabled hover:text-text-primary transition"
                    >
                        <X size={12} />
                    </button>
                </div>
            ...
            <div className="hidden md:block md:border md:border-border md:rounded-xl overflow-hidden relative md:bg-background">

                {/* @Mention Autocomplete Dropdown â€” desktop */}
                {mentionResults.length > 0 && (
                    <div
                        ref={mentionDropdownRef}
                        className="absolute bottom-full mb-2 left-4 right-4 bg-surface border border-border-hover rounded-xl shadow-xl overflow-hidden z-50"
                    >
                        <div className="px-3 py-1.5 text-[10px] font-bold text-text-muted uppercase tracking-wider border-b border-border">
                            Nutzer erwÃ¤hnen
                        </div>
                ...
                {state.error && (
                    <div className="absolute -top-12 left-0 right-0 mx-auto w-fit bg-error/90 text-text-primary px-4 py-2 rounded-lg text-xs font-bold shadow-lg animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="flex flex-col gap-2 px-4 pt-3 pb-1 empty:hidden">
                    {(detectedBrew || isLoadingBrew) && (
                        <div className="flex-1 min-w-0 p-2 bg-background/50 border border-border rounded-lg flex items-center gap-3 animate-in fade-in slide-in-from-bottom-1">
                            {isLoadingBrew ? (
                                <>
                                    <Loader2 className="animate-spin text-text-muted" size={16} />
                                    <span className="text-xs text-text-muted font-bold">Lade Rezept-Details...</span>
                                </>
                            ) : (
                                <>
                                    <div className="w-8 h-8 bg-surface rounded border border-border overflow-hidden flex-shrink-0">
                                        {detectedBrew.image_url ? (
                                            <img src={detectedBrew.image_url} className="w-full h-full object-cover" />
                                        ) : (
                                            <div className="w-full h-full flex items-center justify-center bg-surface">
                                                <Beaker className="w-3 h-3 text-text-disabled" />
                                            </div>
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <span className="text-[10px] uppercase font-bold text-success bg-success/10 px-1.5 py-0.5 rounded">Erkannt</span>
                                            <span className="text-xs font-bold text-text-secondary truncate">{detectedBrew.name}</span>
                                        </div>
                                        {detectedBrew.brewery && (
                                            <div className="text-[10px] text-text-muted truncate">{detectedBrew.brewery.name}</div>
                                        )}
                                    </div>
                                    <LinkIcon size={14} className="text-text-disabled mr-2" />
                                </>
                            )}
                        </div>
                    )}
                    ...
                    {replyTarget && (
                        <div className="flex-1 min-w-0 p-2 bg-success-bg/40 border border-success/20 rounded-lg flex items-start gap-3 relative animate-in slide-in-from-bottom-2">
                            <div className="p-1 bg-success/10 rounded text-success mt-0.5">
                                <Reply size={12} />
                            </div>
                            <div className="flex-1 min-w-0 pr-6">
                                <div className="text-[10px] font-bold text-success uppercase tracking-wider mb-0.5">Antwort auf {replyTarget.authorName}</div>
                                <div className="text-xs text-text-secondary truncate line-clamp-1 italic opacity-80">
                                    "{(() => {
                            ...
                            <button
                                onClick={() => setReplyTarget(null)}
                                className="absolute right-2 top-2 text-text-muted hover:text-text-primary p-1 hover:bg-foreground/5 rounded-full transition"
                            >
                                <X size={12} />
                            </button>
                        </div>
                    ...
                    <div className="flex gap-3 pt-0.5">
                        <div className="flex-1">
                            <textarea
                                ...
                                className="w-full bg-transparent text-text-primary placeholder:text-text-disabled focus:outline-none resize-none min-h-[72px] py-3 text-sm"
                            />
                        </div>
                        <button
                            type="submit"
                            disabled={isPending}
                            className="bg-success hover:bg-success/90 text-white p-2.5 rounded-lg transition disabled:opacity-50 self-end"
                        >
                            {isPending ? <div className="w-5 h-5 border-2 border-text-primary/60 border-t-transparent rounded-full animate-spin" /> : <Send size={18} />}
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div className="md:hidden relative">
                ...
                {mentionResults.length > 0 && (
                    <div
                        ref={mentionDropdownRef}
                        className="absolute bottom-full mb-2 left-0 right-0 bg-surface border border-border-hover rounded-xl shadow-xl overflow-hidden z-50"
                    >
                        {mentionResults.map((user, idx) => (
                            <button
                                key={user.id}
                                ...
                                className={`w-full text-left px-4 py-2.5 text-sm transition flex items-center gap-2 ${
                                    idx === mentionSelectedIdx ? 'bg-success/20 text-success' : 'text-text-secondary'
                                }`}
                            >
                                <span className="text-text-muted text-xs">@</span>
                                <span className="font-medium">{user.display_name}</span>
                ...
                <form ref={formRef} action={(formData) => {
                    ...
                    <input type="hidden" name="threadId" value={threadId} />
                    {/* Pill input area */}
                    <div className="flex-1 bg-surface-hover/80 rounded-3xl px-4 py-2.5 min-h-[44px] flex items-center">
                        <textarea
                            ref={textareaRef}
                            name="content"
                            ...
                            onKeyDown={handleKeyDown}
                            rows={1}
                            className="w-full bg-transparent text-text-primary placeholder:text-text-muted focus:outline-none resize-none text-sm leading-snug max-h-32"
                    ...
                    <button
                        ...
                        className="w-10 h-10 bg-success hover:bg-success/90 text-white rounded-full flex items-center justify-center transition disabled:opacity-40 shrink-0"
                    >
                        {isPending ? <div className="w-4 h-4 border-2 border-text-primary/60 border-t-transparent rounded-full animate-spin" /> : <Send size={16} />}
                    </button>
                </form>
            </div>
        </div>

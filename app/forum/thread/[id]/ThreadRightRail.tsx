...
export default function ThreadRightRail({ poll, brew, tags, currentUserId }: ThreadRightRailProps) {
    ...
    return (
        <aside className="hidden xl:flex w-72 flex-shrink-0 flex-col border-l border-border sticky top-14 self-start pt-6 pb-10 pr-6 lg:pr-10 pl-6 space-y-7 max-h-[calc(100vh-56px)] overflow-y-auto">
            ...
            {brew && (
                <div>
                    ...
                    <Link href={`/brew/${brew.id}`} className="block rounded-xl overflow-hidden hover:bg-surface/50 transition group -mx-2 px-2 pb-2">
                        {/* Image */}
                        <div className="w-full h-32 rounded-lg bg-surface overflow-hidden mb-3">
                            {brew.image_url ? (
                                <img
                                    src={brew.image_url}
                                    alt={brew.name ?? ''}
                                ...
                                <div className="w-full h-full flex items-center justify-center">
                                    <Beaker className="w-8 h-8 text-text-disabled" />
                                </div>
                            )}
                        </div>

                        {/* Name + brewery */}
                        <p className="font-bold text-sm text-foreground group-hover:text-success transition leading-snug">
                            {brew.name}
                        </p>
                        {brew.brewery?.name && (
                            <p className="text-[11px] text-text-muted mt-0.5">{brew.brewery.name}</p>
                        )}
                        ...
                        {(brew.style || brew.brew_type) && (
                            <div className="flex flex-wrap gap-1 mt-2">
                                {brew.style && (
                                    <span className="text-[10px] font-medium px-2 py-0.5 rounded bg-surface-hover/80 text-text-secondary">
                                        {brew.style}
                                    </span>
                                )}
                                {brew.brew_type && (
                        ...
                        {(brew.likes_count ?? 0) > 0 && (
                            <div className="flex items-center gap-1 mt-2 text-[11px] text-text-disabled">
                                <Heart size={10} />
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {tags.map(tag => (
                            <span
                                key={tag}
                                className="text-[11px] font-medium px-2 py-0.5 rounded bg-surface-hover/80 text-text-muted"
                            >
                                {tag}
                            </span>
                        ))}

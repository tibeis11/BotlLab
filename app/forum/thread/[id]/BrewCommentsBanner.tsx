...
export default function BrewCommentsBanner({ brew }: BrewCommentsBannerProps) {
  ...
  return (
    <div className="bg-brand-bg/30 border-b border-brand/30 px-6 md:px-12 lg:px-16 py-3 max-w-screen-2xl mx-auto">
      <div className="flex items-center gap-3">
        <div className="w-7 h-7 rounded-lg bg-brand/20 border border-brand/40 flex items-center justify-center shrink-0">
        ...
        <p className="text-xs text-brand/80">
          ...
          <span className="font-bold text-brand">{brew.name ?? brew.id}</span>.
        </p>
        <Link
          href={`/brew/${brew.id}?tab=kommentare`}
          className="ml-auto shrink-0 flex items-center gap-1 text-[11px] font-bold text-brand hover:text-brand transition"
        >

    ...
    return (
        <>
            ...
            <div className="relative overflow-hidden bg-surface border-b border-border">
                <div className="absolute inset-0 bg-gradient-to-r from-background/95 via-background/80 to-brand-bg/20" />
                <div className="relative px-6 md:px-12 lg:px-16 py-6 md:py-10 max-w-screen-2xl mx-auto">
                    {/* Breadcrumb */}
                    <Link
                        href="/forum"
                        className="inline-flex items-center gap-1.5 text-xs text-text-disabled hover:text-text-secondary mb-4 transition-colors"
                    >
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <p className="text-brand font-bold uppercase tracking-widest text-xs">Community Forum</p>
                    </div>
                    <h1 className="text-2xl md:text-4xl font-black tracking-tight mb-2 leading-tight">
                        Rezept-<span className="text-brand">Kommentare</span>
                    </h1>
                ...
                <main className="flex-1 min-w-0 px-6 pt-8 pb-16">
                    ...
                    {threads.length === 0 ? (
                        <div className="text-center py-20">
                            <FlaskConical className="w-10 h-10 text-text-disabled mx-auto mb-4" />
                            <p className="font-bold text-text-secondary mb-1">Noch keine Rezept-Diskussionen</p>
                            <p className="text-sm text-text-disabled mb-5">
                                Kommentiere ein Rezept auf der Brew-Seite, um die erste Diskussion zu starten.
                            </p>
                            <Link
                                href="/discover"
                                className="inline-flex items-center gap-2 text-sm text-brand hover:text-brand font-bold transition-colors"
                            >
                                Brews entdecken →
                            </Link>
                        </div>
                        ...
                        <>
                            <p className="text-[11px] font-bold text-text-disabled uppercase tracking-widest mb-4">
                                Zuletzt aktiv
                            </p>
                                ...
                                {threads.map((thread: any) => {
                                    ...
                                    return (
                                        <Link
                                            key={thread.id}
                                            href={href}
                                            className="flex flex-col gap-2 bg-surface/60 hover:bg-surface border border-border hover:border-brand/30 rounded-2xl p-3 transition-colors group"
                                        >
                                            {/* Brew image */}
                                            <div className="w-full aspect-square rounded-xl bg-surface-hover border border-border-hover overflow-hidden flex items-center justify-center">
                                                {brew?.image_url ? (
                                                    <img
                                                        src={brew.image_url}
                                                        alt={brew.name ?? ''}
                                                    ...
                                                    <FlaskConical className="w-8 h-8 text-brand/30" />
                                                )}
                                            </div>

                                            {/* Info */}
                                            <div className="space-y-0.5 flex-1">
                                                <p className="text-xs font-bold text-foreground group-hover:text-text-primary line-clamp-2 leading-snug transition-colors">
                                            ...
                                            <div className="flex items-center gap-1 pt-1 border-t border-border">
                                                <MessageSquare className="w-3 h-3 text-text-disabled group-hover:text-brand transition-colors" />

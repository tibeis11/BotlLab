...
export default function TeamFeedPage({ params }: { params: Promise<{ breweryId: string }> }) {
    ...
    if (user && breweryId) {
      ...
      const channel = supabase
        .channel('feed_updates')
         // Listen to ALL feed changes for this brewery
        .on(
          'postgres_changes',
  ...
  function renderFeedItem(item: FeedItem) {
    ...
    const getTierBorderColor = (tier?: string) => {
        if (!tier) return 'border-zinc-500'; 
        if (tier === 'free') return 'border-zinc-500'; 
        if (tier === 'brewer') return 'border-blue-500';
        if (tier === 'brewery') return 'border-amber-500'; 
        if (tier === 'enterprise') return 'border-purple-500';
        return 'border-zinc-500';
    };

    const tierBorderColor = getTierBorderColor(item.profiles?.subscription_tier);
            ...
            <div key={item.id} className="flex flex-col items-center justify-center gap-1 py-4 text-sm text-zinc-500 animate-in fade-in zoom-in-95 duration-300 w-full px-4">
                 <div className="flex flex-col gap-2 bg-zinc-900/80 px-4 py-3 rounded-2xl border border-zinc-800 shadow-sm backdrop-blur-sm max-w-full md:max-w-xl w-full">
                    ...
                    <div className="flex items-center gap-2 border-b border-white/5 pb-2 mb-1">
                        <div className={`w-6 h-6 rounded-full bg-zinc-800 border-2 ${tierBorderColor} flex items-center justify-center overflow-hidden flex-shrink-0`}>
    ...
    return (
      <div key={item.id} className={`flex gap-4 mb-6 ${isMe ? 'flex-row-reverse' : ''} animate-in slide-in-from-bottom-2 duration-300 group`}>
        ...
        <div className="flex-shrink-0 -mt-1">
          <div className={`w-10 h-10 rounded-full bg-zinc-900 border-2 ${tierBorderColor} overflow-hidden shadow-lg`}>
  ...
  // Calculate messages in last 24h or total visible
  const messageCount = feed.filter(f => f.type === 'POST').length;
  const recentActivity = feed.length; 
    ...
    <div className="space-y-8 pb-24">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-zinc-900 pb-8">
            <div>
                <div className="flex items-center gap-3 mb-1">
                    <h1 className="text-2xl font-bold text-white tracking-tight">Squad Feed</h1>
                    <span className="text-[10px] font-black px-2 py-0.5 rounded uppercase border border-cyan-500/30 text-cyan-400 bg-cyan-500/10 tracking-wider shadow-[0_0_10px_rgba(34,211,238,0.2)] animate-pulse">
                        LIVE
                    </span>
                </div>
                <p className="text-sm text-zinc-500">Der Stammtisch für deinen Squad. Diskutiere, teile und feiere.</p>
            </div>
            
            <div className="flex items-center gap-4 text-zinc-500 text-xs">
                 {/* Place for extra header items if needed */}
            </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8 items-start relative">
         ...
         <div className="space-y-6 lg:sticky lg:top-8 z-20">
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="bg-zinc-900/30 border border-zinc-800 p-4 rounded-xl flex flex-col justify-between h-24 relative overflow-hidden group hover:border-cyan-500/30 transition-colors">
                    ...
                    <div className="text-cyan-500 text-xs font-bold uppercase tracking-wider relative z-10">Nachrichten</div>
                    <div className="text-2xl font-mono font-bold text-cyan-400 relative z-10">{messageCount}</div>
                </div>
                <div className="bg-zinc-900/30 border border-zinc-800 p-4 rounded-xl flex flex-col justify-between h-24 relative overflow-hidden group hover:border-purple-500/30 transition-colors">
                    ...
                    <div className="text-2xl font-mono font-bold text-purple-400 relative z-10">
                        {memberCount}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-4">
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between text-zinc-400 text-xs font-bold uppercase tracking-wider">
                        ...
                        <span className="text-white font-mono">{recentActivity}</span>
                    </div>
                    ...
                    <p className="text-[10px] text-zinc-500 text-center pt-1">
                        Der Feed aktualisiert sich automatisch live.
                    </p>
                </div>
            </div>
         ...
         <div className="space-y-8">
            ...
            {/* Input Area */}
            <div className="bg-zinc-900/30 backdrop-blur-md border border-zinc-800/60 p-6 rounded-3xl shadow-2xl relative overflow-hidden group focus-within:ring-2 focus-within:ring-cyan-500/20 transition-all">
                ...
                <form onSubmit={handlePost} className="relative z-10">
                    <textarea
                        ...
                        className="w-full bg-zinc-950/40 border border-zinc-800/50 focus:border-cyan-500/50 p-5 rounded-2xl text-white placeholder-zinc-500 resize-none focus:outline-none min-h-[100px] text-base mb-4 transition-all"
                        onKeyDown={(e) => {
                    ...
                    <div className="flex justify-between items-center pl-2">
                        ...
                        <button 
                            ...
                            className="bg-cyan-500 text-black hover:bg-cyan-400 disabled:opacity-50 disabled:cursor-not-allowed font-black px-8 py-2.5 rounded-xl text-xs transition-all shadow-lg hover:shadow-cyan-500/20 flex items-center gap-2 transform active:scale-95 uppercase tracking-wide"
                        >
                            {isPosting ? 'Sende...' : (

...
export function ConditioningTab() {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            ...
            {/* Header */}
            <div>
                <h2 className="text-2xl font-bold text-text-primary tracking-tight">Reifung & Karbonisierung</h2>
                <p className="text-text-muted text-sm">Flaschengärung und Reifeprozess für <span className="font-mono text-text-muted">#{session?.batch_code || session?.brew?.name}</span></p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-surface border border-border rounded-xl p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                             ...
                             <div>
                                ...
                                <input 
                                    ...
                                    className="w-full bg-background border border-border rounded-lg px-3 py-2 h-10 text-sm text-text-primary font-mono outline-none focus:border-brand focus:ring-1 focus:ring-brand/20 transition"
                                />
                             </div>
                        ...
                        {/* Result Banner */}
                        <div className="mt-6 bg-emerald-900/10 border border-emerald-500/20 rounded-xl p-4 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-zinc-500 table-nums">Basierend auf {carbVolume}L bei {carbTemp}°C</div>
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-3xl font-mono font-black text-text-primary">{sugarTotal || 0}<span className="text-base font-bold text-text-muted ml-1">g</span></div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    <div className="bg-surface border border-border rounded-xl p-6 h-full">
                        ...
                        {statusData ? (
                            <div className="space-y-6">
                                <div className="text-center py-6 border border-border rounded-xl bg-surface-hover">
                                    <div className={`w-16 h-16 rounded-full mx-auto flex items-center justify-center mb-4 ${statusData.isComplete ? 'bg-emerald-500/20 text-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-orange-500/20 text-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.2)]'}`}>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex justify-between text-xs border-b border-border pb-2">
                                        <span className="text-text-muted">Zieldatum</span>
                                        <span className="text-text-primary font-mono">
                                    ...
                                    <div className="flex justify-between text-xs">
                                        ...
                                        <span className="text-text-primary font-mono">{statusData.progress.toFixed(0)}%</span>
                                    </div>
                                    <div className="h-1 bg-surface-hover rounded-full overflow-hidden mt-2">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-text-muted block">Dauer (Tage)</label>
                                    <div className="flex items-center gap-2">
                                        <input 
                                            type="number" 
                                            value={durationDays}
                                            onChange={(e) => setDurationDays(e.target.value)} 
                                            className="w-20 bg-background border border-border rounded-lg px-3 py-2 h-10 text-text-primary font-mono outline-none focus:border-brand focus:ring-1 focus:ring-brand/20 transition text-sm text-center"
                                        />
                                        <span className="text-text-disabled text-xs">Empfohlen: 14</span>
                                    </div>
                                </div>

                                <button 
                                    onClick={handleStartConditioning}
                                    className="w-full py-3 bg-surface-hover hover:bg-border-hover text-text-primary text-xs font-bold uppercase tracking-wider rounded-lg border border-border transition-colors flex items-center justify-center gap-2 group"
                                >

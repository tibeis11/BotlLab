...
export default function AppealsView() {
    ...
    if (loading) {
        return <div className="p-12 text-center text-(--text-muted) animate-pulse">Lade Widersprüche...</div>;
    }

    if (appeals.length === 0) {
        return (
            <div className="bg-(--surface) border border-(--border) rounded-3xl p-12 text-center">
                <Scale className="w-12 h-12 opacity-50 mx-auto mb-4 text-(--text-muted)" />
                <h2 className="text-xl font-bold text-(--text-primary) mb-2">Keine offenen Widersprüche</h2>
                <p className="text-(--text-muted)">Alle Widersprüche wurden bearbeitet.</p>
                <button
                    onClick={loadAppeals}
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button onClick={loadAppeals} className="text-(--text-muted) hover:text-(--text-primary) text-sm">⇻ Refresh</button>
            </div>
            ...
            <div className="grid gap-4">
                {appeals.map((appeal) => (
                    <div key={appeal.id} className="bg-(--surface) border border-(--border) rounded-xl overflow-hidden">
                        {/* Header */}
                        <div className="px-4 py-3 border-b border-(--border)/60 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-(--surface-hover) flex items-center justify-center overflow-hidden border border-(--border-hover)">
                                ...
                                <div>
                                    <span className="font-bold text-(--text-secondary) text-sm">{appeal.user?.display_name || 'Unbekannt'}</span>
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded bg-(--surface-hover) text-(--text-secondary) border border-(--border-hover)">
                        ...
                        <div className="p-4 space-y-3">
                            {appeal.target_title && (
                                <div className="text-xs text-(--text-muted)">
                                    Betroffener Inhalt: <span className="text-(--text-secondary) font-medium">&quot;{appeal.target_title}&quot;</span>
                                </div>
                            )}

                            <div className="bg-(--surface-sunken)/60 border border-(--border) rounded-lg p-3">
                                <div className="text-[10px] uppercase font-bold tracking-wider text-(--text-disabled) mb-1">Begründung des Nutzers:</div>
                                <p className="text-sm text-(--text-secondary) whitespace-pre-wrap">{appeal.appeal_text}</p>
                            </div>
                            ...
                            {responding === appeal.id ? (
                                <div className="space-y-3 pt-2">
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full bg-(--surface-sunken) border border-(--border-hover) rounded-xl p-3 text-sm text-(--text-primary) placeholder:text-(--text-disabled) focus:outline-none focus:border-(--text-muted) resize-y"
                                    />
                                    {error && <p className="text-xs text-red-400">{error}</p>}
                                    <div className="flex gap-2 justify-end">
                                        <button
                                            onClick={() => { setResponding(null); setResponseText(''); setError(null); }}
                                            className="px-3 py-2 text-xs font-bold text-(--text-secondary) hover:text-(--text-primary) border border-(--border-hover) rounded-lg transition"
                                        >
                                            Abbrechen
                                        </button>
                                        <button
                                            onClick={() => handleResolve(appeal.id, 'rejected')}
                                            className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold bg-red-600 hover:bg-red-500 text-(--text-primary) rounded-lg transition"
                                        >
                                            <XCircle size={13} /> Ablehnen
                                        </button>
                                        <button
                                            onClick={() => handleResolve(appeal.id, 'accepted')}
                                            className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-(--text-primary) rounded-lg transition"
                                        >

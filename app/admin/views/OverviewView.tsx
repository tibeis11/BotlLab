...
import { AlertTriangle, Shield, Mail, RefreshCw, ArrowRight, Users, Flame, Beer, Camera, Star } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <MetricCard title="Total Nutzer" value={(summary?.totalUsers ?? 0).toLocaleString()} icon={<Users className="w-4 h-4" />} />
        <MetricCard title="Aktive Nutzer" value={(summary?.activeUsers ?? 0).toLocaleString()} icon={<Flame className="w-4 h-4" />} />
        <MetricCard title="Brews gesamt" value={(summary?.totalBrews ?? 0).toLocaleString()} icon={<Beer className="w-4 h-4" />} />
        <MetricCard title="QR-Scans" value={(summary?.totalScans ?? 0).toLocaleString()} icon={<Camera className="w-4 h-4" />} />
        <MetricCard title="Fehler (24h)" value={summary?.errorCount ?? 0} icon={<AlertTriangle className="w-4 h-4" />} />
        <MetricCard title="Ø Bewertung" value={`${summary?.avgRating?.toFixed(1) ?? '—'}`} icon={<Star className="w-4 h-4" />} />
      </div>

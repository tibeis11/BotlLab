...
function StatusBadge({ count, label }: { count: number; label: string }) {
  ...
  const status = count >= 200 ? 'Robust' : count >= 50 ? 'Ausreichend' : 'Zu wenig'
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-(--text-primary)">Model Health: Scan Intent Classification</h2>
          <p className="text-xs text-(--text-muted)">Phase 9 — Überwachung der Intent-Klassifikation</p>
        </div>
      </div>
          ...
          {alerts.map((alert, i) => {
            const colors = alert.severity === 'critical'
              ...
              : alert.severity === 'warning'
                ...
                : 'bg-(--surface) border-(--border-hover) text-(--text-secondary)'
            ...
            return (
              <div key={i} className={`flex items-start gap-3 p-3 rounded-lg border ${colors}`}>
                ...
                {alert.intent && (
                  <span className="text-[10px] bg-(--surface-hover) px-1.5 py-0.5 rounded text-(--text-secondary) flex-shrink-0">
                    {alert.intent}
                  </span>
                )}
              </div>
      ...
      <div className="bg-(--surface) rounded-lg border border-(--border) p-6">
        <h3 className="text-sm font-medium text-(--text-secondary) mb-4">Confusion Matrix (Gesamt)</h3>

        <div className="overflow-x-auto">
          <table className="w-full max-w-md mx-auto text-xs">
            <thead>
              <tr>
                <th className="text-left text-(--text-muted) pb-2" />
                <th className="text-center text-(--text-secondary) pb-2 px-4">Nutzer: &quot;Ja&quot;</th>
                <th className="text-center text-(--text-secondary) pb-2 px-4">Nutzer: &quot;Nein&quot;</th>
                <th className="text-right text-(--text-muted) pb-2" />
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="text-(--text-secondary) pr-4 py-2 font-medium">Modell: Trinker</td>
                ...
                <td className="text-right pl-4 py-2 text-(--text-muted)">Prec: {pct(overall.precision)}</td>
              </tr>
              <tr>
                <td className="text-(--text-secondary) pr-4 py-2 font-medium">Modell: Kein Trinker</td>
                <td className="text-center py-2">
                ...
                <td className="text-right pl-4 py-2 text-(--text-muted)">
                  NPV: {computeNPV(perIntent)}
                </td>
              </tr>
              <tr>
                <td className="pt-2" />
                <td className="text-center pt-2 text-(--text-muted)">Recall: {pct(overall.recall)}</td>
                <td className="text-center pt-2 text-(--text-muted)">
        ...
        <div className="text-center mt-4 space-y-1">
          <p className="text-xs text-(--text-secondary)">
            Gesamt-Accuracy: <span className="text-(--text-primary) font-mono font-medium">{pct(overall.accuracy)}</span>
            {' '} · F1-Score: <span className="text-(--text-primary) font-mono font-medium">{overall.f1Score.toFixed(3)}</span>
          </p>
        </div>
      </div>
      ...
      <div className="bg-(--surface) rounded-lg border border-(--border) p-6">
        <h3 className="text-sm font-medium text-(--text-secondary) mb-4">Aufschlüsselung nach Intent</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {perIntent.map((pi) => (
                <tr key={pi.intent} className="border-b border-(--border)/50 hover:bg-(--surface-hover)/30">
                  <td className="py-2 pr-3 font-medium text-(--text-secondary)">{pi.intent}</td>
                  ...
                  <td className="text-right py-2 px-2 font-mono text-(--text-secondary)">{pct(pi.accuracy)}</td>
                  ...
                  <td className="text-right py-2 px-2 font-mono text-(--text-secondary)">{pi.feedbackCount}</td>
                  <td className="text-right py-2 px-2 font-mono text-(--text-muted)">{pi.currentProbability}</td>
      ...
      {calibrationCurve.length > 0 && (
        <div className="bg-(--surface) rounded-lg border border-(--border) p-6">
          <h3 className="text-sm font-medium text-(--text-secondary) mb-4">Calibration Curve</h3>
            ...
            <table className="w-full max-w-lg mx-auto text-[11px]">
              ...
              <tbody>
                {calibrationCurve.map((c) => {
                  ...
                  return (
                    <tr key={c.bin} className="border-b border-(--border)/50">
                      ...
                      <td className="text-right py-1.5 font-mono text-(--text-muted)">{c.n}</td>
                      <td className="py-1.5 pl-4">
                        <div className="flex items-center gap-1">
                          <div className="w-20 h-1.5 bg-(--surface-hover) rounded-full overflow-hidden relative">
                            <div
      ...
      {drift.length > 0 && (
        <div className="bg-(--surface) rounded-lg border border-(--border) p-6">
          <h3 className="text-sm font-medium text-(--text-secondary) mb-4">Accuracy-Trend (7-Tage Rolling Window)</h3>
                ...
                <tbody>
                  {drift.slice(-14).map((d) => (
                    <tr key={d.date} className="border-b border-(--border)/30">
                      <td className="py-1 text-(--text-secondary) font-mono">{d.date}</td>
      ...
      {/* Sample Coverage */}
      <div className="bg-(--surface) rounded-lg border border-(--border) p-6">
        <h3 className="text-sm font-medium text-(--text-secondary) mb-4">Stichproben-Abdeckung</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {perIntent.map(pi => (
                <tr key={pi.intent} className="border-b border-(--border)/50 hover:bg-(--surface-hover)/30">
                  <td className="py-2 text-(--text-secondary) font-medium">{pi.intent}</td>
                  <td className="text-right py-2 font-mono text-(--text-secondary)">{pi.feedbackCount}</td>
      ...
      <div className="bg-(--surface) rounded-lg border border-(--border) p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-medium text-(--text-secondary)">Phase-0 Scoring-Modell</h3>
          <span className="ml-auto text-[10px] text-(--text-disabled)">Aktuelle Konfiguration der CIS Engine v2</span>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            { label: 'Base Score', value: '0.30', color: 'text-(--text-secondary)', desc: 'Startwert jeder QR-Session' },
        ...
        <div className="mt-4 text-[10px] text-(--text-disabled) border-t border-(--border) pt-3">
          ...
          ≥ 0.45 → <code className="text-(--text-secondary)">single</code>
        </div>
      </div>
      ...
      <div className="bg-(--surface) rounded-lg border border-(--border) p-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-sm font-medium text-(--text-secondary)">False-Negative Tracker</h3>
              ...
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {falseNegs.examples.map(ex => (
                    <tr key={ex.scanId} className="border-b border-(--border)/40 hover:bg-(--surface-hover)/30">
                      <td className="py-1.5 text-(--text-secondary) font-medium max-w-[160px] truncate">
                        {ex.brewName}
                      </td>
                      <td className="text-right py-1.5 font-mono text-(--text-secondary)">{ex.scanIntent}</td>
                      ...
                      <td className="text-right py-1.5 font-mono text-(--text-disabled)">
...
function KPITile({ label, value, icon, showTrend, trendValue, subtitle }: {
  ...
  return (
    <div className="bg-(--surface) rounded-lg border border-(--border) p-5 flex flex-col justify-between hover:border-(--border-active) transition-colors">
      <div>
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-2xl font-medium text-(--text-primary) font-mono">{value}</span>
      ...
      {subtitle && (
        <p className="text-[10px] text-(--text-disabled) mt-2">{subtitle}</p>
      )}
    </div>
  )
}

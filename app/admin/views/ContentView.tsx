...
import { Star, TrendingUp, Zap, AlertTriangle, ToggleLeft, ToggleRight, Search, X, Pin, PinOff, Beer, FileText, Wine } from 'lucide-react'
      ...
      <div className="pt-4 border-t border-zinc-800">
        ...
        {adminLoading ? (
          ...
          <div className="space-y-6">
            ...
            {/* B+C: Unified Brew Manager — Trending Override + Featured */}
            <div className="bg-black border border-zinc-800 rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-cyan-400" />
                  <Zap className="w-4 h-4 text-purple-400" />
                  <h3 className="text-sm font-medium text-white">Brew Manager</h3>
                </div>
                <span className="text-xs text-zinc-500">
                  <span className="text-purple-400 font-bold">{featuredBrews.length}</span> featured
                  {' · '}
                  <span className="text-cyan-400 font-bold">{adminBrewList.filter(b => b.trending_score_override !== null).length}</span> gepinnt
                </span>
              </div>
              <p className="text-xs text-zinc-500 mb-5">
                <span className="text-cyan-400 font-medium">Trending-Pin:</span> Setzt einen festen Score — wird vom Cron <span className="text-white font-medium">nicht überschrieben</span>.{' '}
                <span className="text-purple-400 font-medium">Featured:</span> Brews erscheinen oben in der Discover-Page (max. 6 empfohlen).
              </p>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-zinc-800 text-left">
                      ...
                      <th className="py-2 px-3 text-right text-zinc-500">Trending</th>
                      <th className="py-2 px-3 text-zinc-500">Score pinnen</th>
                      <th className="py-2 px-3 text-center text-zinc-500">Featured</th>
                    </tr>
                  </thead>
                  <tbody>
                    {adminBrewList.map(brew => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-2 px-3 text-white font-medium max-w-[180px]">
                            <div className="flex items-center gap-1.5 truncate">
                              {isPinned && <Pin className="w-3 h-3 text-cyan-400 shrink-0" />}
                          ...
                          <td className="py-2 px-3 text-right">
                            ...
                            {isPinned && (
                              <span className="ml-1.5 text-[10px] text-cyan-500 font-bold uppercase tracking-wide">pin</span>
                            )}
                          </td>
                          <td className="py-2 px-3">
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                onChange={e =>
                                  setTrendingOverrides(prev => ({ ...prev, [brew.id]: e.target.value }))
                                }
                                className="w-24 bg-zinc-900 border border-zinc-700 rounded px-2 py-1 text-white text-xs focus:outline-none focus:border-cyan-500"
                              />
                              <button
                                onClick={() => handleTrendingOverride(brew.id, brew.name)}
                                disabled={!trendingOverrides[brew.id] || savingTrending === brew.id}
                                className="px-2.5 py-1 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-30 disabled:cursor-not-allowed text-white rounded font-bold transition text-xs"
                              >

...
export default function EmailReportsView() {
    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="text-center">
          ...
          <p className="text-zinc-400">Lade Email-Report-Daten…</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-white">E-Mail-Reports</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        />
        <MetricCard title="Wöchentlich" value={stats.weeklySubscriptions} icon="📅" />
        <MetricCard title="Monatlich" value={stats.monthlySubscriptions} icon="📆" />
        <MetricCard
          title="Gesendet (30d)"
        ...
        />
        <MetricCard title="Erfolgsrate" value={`${sentPct}%`} icon="📊" />
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-zinc-300 uppercase tracking-wider">Letzte Report-Zustellungen</h2>
          <span className="text-xs text-zinc-500">{logs.length} Einträge</span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-800">
                ...
                <th className="px-4 py-3 text-left text-xs text-zinc-500 font-medium uppercase tracking-wider">Zeitraum</th>
                <th className="px-4 py-3 text-left text-xs text-zinc-500 font-medium uppercase tracking-wider">E-Mail</th>
                <th className="px-4 py-3 text-left text-xs text-zinc-500 font-medium uppercase tracking-wider">Fehlermeldung</th>
                <th className="px-4 py-3 text-left text-xs text-zinc-500 font-medium uppercase tracking-wider">Erstellt</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-800/50">
              {logs.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-zinc-500">Noch keine Einträge vorhanden</td>
                </tr>
              ) : (
                logs.map(log => {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-zinc-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3 text-zinc-400 text-xs font-mono">{log.emailMasked ?? '—'}</td>
                      <td className="px-4 py-3 text-zinc-500 text-xs max-w-xs truncate">{log.errorMessage ?? '—'}</td>

    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Alle Admin-Aktionen werden hier protokolliert (letzten 200)</p>
        </div>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="p-8 text-center text-zinc-500">Keine Einträge gefunden.</div>
        ) : (
          <div className="divide-y divide-zinc-800/50">
            {filtered.map(log => (
              <div key={log.id}>
                <div
                  className="flex items-start gap-3 p-4 hover:bg-zinc-800/20 cursor-pointer"
                ...
                {expandedId === log.id && (
                  <div className="px-4 pb-4 bg-zinc-950/40">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div>
                        <p className="text-zinc-600 mb-1 uppercase tracking-wider text-[10px]">Admin ID</p>
                        <p className="font-mono text-zinc-400">{log.admin_id || '–'}</p>
                      </div>
                      <div>
                        <p className="text-zinc-600 mb-1 uppercase tracking-wider text-[10px]">IP-Adresse</p>
                        <p className="font-mono text-zinc-400">{log.ip_address || 'nicht erfasst'}</p>
                      </div>
                      <div>
                        <p className="text-zinc-600 mb-1 uppercase tracking-wider text-[10px]">Ressource</p>
                        <p className="font-mono text-zinc-400 break-all">{log.resource_id || '–'}</p>
                      </div>
                      <div>
                        <p className="text-zinc-600 mb-1 uppercase tracking-wider text-[10px]">User-Agent</p>
                        <p className="font-mono text-zinc-400 truncate" title={log.user_agent || undefined}>
                          {log.user_agent || 'nicht erfasst'}
                        </p>
                      </div>
                      {log.details && Object.keys(log.details).length > 0 && (
                        <div className="sm:col-span-2">
                          <p className="text-zinc-600 mb-1 uppercase tracking-wider text-[10px]">Details</p>
                          <pre className="bg-black/50 rounded p-2 text-zinc-400 text-[10px] overflow-x-auto">
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4 text-xs text-zinc-500">
        <p className="font-medium text-zinc-400 mb-1">Datenschutzhinweis</p>
        <p>

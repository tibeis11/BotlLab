-- ══════════════════════════════════════════════════════════════════════════════
-- Stage 5: Context-Anreicherung — Recipe Success Score
--
-- Updates get_user_brew_context to include community rating data:
--   - avgRating + ratingCount per brew in recentBrews
--   - topRatedBrew → best-rated brew with key recipe parameters
-- ══════════════════════════════════════════════════════════════════════════════
  ...
  SELECT jsonb_build_object(
    ...
    'topRatedBrew', (
      ...
      WHERE b.user_id = p_user_id
        AND r.moderation_status = 'auto_approved'
      GROUP BY b.id, b.name, b.style, b.og, b.ibu
      HAVING COUNT(r.id) >= 2
